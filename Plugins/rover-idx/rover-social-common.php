...
class roveridx_social		{
		...
		if (is_array($this->settings))
			...
			foreach(array('post_to_wp', 'post_to_fb', 'post_to_gp', 'post_to_tw') as $key)
				...
				if (is_bool($this->settings[$key]))
					$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, $key.' is bool ['.(($this->settings[$key] === true) ? 'true' : 'false').']');
				else if (is_numeric($this->settings[$key]))
					$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, $key.' is numeric ['.$this->settings[$key].']');
				else if (is_string($this->settings[$key]))
					$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, $key.' is string ['.$this->settings[$key].']');
				}

			$this->wp_is_enabled			= (isset($this->settings['post_to_wp']) && $this->settings['post_to_wp'] === true) ? true : false;
			$this->fb_is_enabled			= (isset($this->settings['post_to_fb']) && $this->settings['post_to_fb'] === true) ? true : false;
			$this->gp_is_enabled			= (isset($this->settings['post_to_gp']) && $this->settings['post_to_gp'] === true) ? true : false;
			$this->tw_is_enabled			= (isset($this->settings['post_to_tw']) && $this->settings['post_to_tw'] === true) ? true : false;
	...
	private function get_last_search_date()	{
		if (is_null($this->last_search_date))
			...
			if (is_object($this->settings))
				$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, 'this->settings is object');
			else if (is_array($this->settings))
				$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, 'this->settings is array');
	...
	private function get_updated_properties($search_since)	{
		...
		$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, 'There are '.count($rover_idx->all_selected_regions).' selected regions ['.implode(',', array_keys($rover_idx->all_selected_regions)).']');
		...
		foreach ($rover_idx->all_selected_regions as $one_region => $region_slugs)
			...
			$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, '['.$rover_content['the_html'].']');
			...
			if (is_array($result_decoded) && isset($result_decoded['properties']) && count($result_decoded['properties']))
				...
				foreach ($result_decoded['properties'] as $prop)
					{
					$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, '['.$prop['id'].'] ['.$prop['mlnumber'].']');

					if (is_array($prop))
						$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, 'prop is [array] of ['.count($prop).'] items');
					else if (is_object($prop))
						$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, 'prop is [object] of ['.count($prop).'] items');
...
//				add_post_meta($id, '_thumbnail_id', $attach_id, true);
				...
				add_post_meta($id, 'rover-idx-mlnumber', $one_property['mlnumber']);

				$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, 'Requesting ['.$one_property['image'].']');

				roveridx_update_tracking($one_property['id'], $one_property['region']);

				$request						= new WP_Http();
				$photo							= $request->request( $one_property['image'] );

				if (is_a($photo, 'WP_Error'))
					{
					$this->rover_error_social_log(__FILE__, __FUNCTION__, __LINE__, 'WP_Http request returned an error: '.$photo['response']['code']);
					}
				else
					{
